-- Update user_can_write_tables to remove delivery
...
-- Update user_can_write_risk_appetite to remove delivery
...
AS $$
  SELECT EXISTS (
    ...
    WHERE user_id = _user_id
    AND project_id = _project_id
    AND role IN ('risk_manager')
  ) OR user_is_security_admin()
$$;

-- Drop and recreate user_can_write_threats to remove delivery
...
AS $$
  SELECT EXISTS (
    ...
    WHERE project_id = _project_id
      ...
      AND role IN ('security_architect', 'risk_manager', 'sec_mon', 'sec_eng')
  )
$$;
